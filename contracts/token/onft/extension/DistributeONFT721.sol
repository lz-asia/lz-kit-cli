...
contract DistributeONFT721 is ONFT721 {
    ...
    ) public view virtual override(IONFT721Core, ONFT721Core) returns (uint nativeFee, uint zroFee) {
    ...
    function distributeTokens(
        uint16 _dstChainId,
        TokenDistribute[] memory _tokenDistribute,
    ...
    function estimateDistributeFee(
        uint16 _dstChainId,
        TokenDistribute[] memory _tokenDistribute,
        bool _useZro
        ...
        return
            _estimatePayloadFee(
                _dstChainId,
                abi.encode(FUNCTION_TYPE_DISTRIBUTE, _tokenDistribute),
                _tokenDistribute.length,
                _useZro
            );
    }
    ...
    function _nonblockingLzReceive(
        ...
        if (functionType == FUNCTION_TYPE_SEND) {
            ...
            (, bytes memory _toAddressBytes, uint[] memory _receivedTokenIds) = abi.decode(
                _payload,
                (uint16, bytes, uint[])
            );
